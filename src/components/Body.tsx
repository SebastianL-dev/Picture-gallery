...
export default function MainB() {
  ...
  return (
    <div className="flex flex-col items-center pt-56 pb-36 gap-36">
        ...
        {imageData.map((image) => (
          ...
          >
            ...
            <div className="px-4 py-3 text-white absolute top-0 z-10 opacity-0 group-hover:opacity-100 transition-all ease-out duration-300 flex justify-between w-full cursor-default">
              <button
                className="flex gap-2 bg-amber-500 px-3 py-1 rounded-[4px] skew-x-[-13deg] bg-opacity-60 hover:bg-opacity-80 transition-all ease-linear"
                onClick={() => Download(image)}
              >
                <span className=" skew-x-[13deg]">Descargar</span>
                <MdOutlineFileDownload className="h-6 w-6 skew-x-[13deg]" />
              </button>
            </div>
          </div>
        ))}
